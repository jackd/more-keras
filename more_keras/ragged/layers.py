"""Hopefully this can all be dropped come 2.0."""
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
import tensorflow as tf
from more_keras import visitor


def _actual_ragged_lambda_fn(flat_comp_inputs, fn, input_structure, cache,
                             kwargs):
    ...
    if not (all(isinstance(x, tf.Tensor) for x in flat_comp_inputs)):
        raise Exception('Expected flat_inputs to be tensors, got %s' %
                        str(flat_comp_inputs))
    inner_comp_inputs = tf.nest.pack_sequence_as(input_structure,
                                                 flat_comp_inputs)
...
def ragged_lambda_call(fn, args, arguments={}, name=None):
    ...
    cache = []
    flat_out = tf.keras.layers.Lambda(_actual_ragged_lambda_fn,
...
def ragged_from_tensor(tensor, lengths=None, **kwargs):
    if lengths is None:
        return ragged_lambda(tf.RaggedTensor.from_tensor,
                             arguments=kwargs)(tensor)
    else:
        return ragged_lambda(_ragged_from_tensor,
                             arguments=kwargs)([tensor, lengths])
...
def ragged_from_nested_row_lengths(flat_values, nested_row_lengths):
    return ragged_lambda(_ragged_from_nested_row_lengths)([flat_values] +
                                                          nested_row_lengths)
...
def ragged_to_tensor(ragged, default_value=None):
    return ragged_lambda(tf.RaggedTensor.to_tensor,
