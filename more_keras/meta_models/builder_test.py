...
from more_keras.meta_models import utils
...
class MetaNetworkBuilderTest(tf.test.TestCase):
    ...
    def test_yield_flat_paths(self):
        ...
        self.assertEqual(tuple(utils.yield_flat_paths(x)),
                         (('a', 0), ('a', 1), ('b',), ('c', 'd', 0)))
        AB = collections.namedtuple('AB', ['a', 'b'])
        self.assertEqual(tuple(utils.yield_flat_paths(AB(2, 3))),
                         (('a',), ('b',)))
        x = [AB(3, 4), 2]
        self.assertEqual(tuple(utils.yield_flat_paths(x)),
                         ((0, 'a'), (0, 'b'), (1,)))
    ...
    def test_batched_ragged_ragged(self):
        self._test_batched_ragged_ragged([[0, 2, 5], [0, 4, 6, 7]], None)
        self._test_batched_ragged_ragged([[0, 2, 5], [0, 4, 6]], 3)
